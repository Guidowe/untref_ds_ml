...
Lista <- list(A,B,individual_t117)
...
La diferencia entre un vector _character_ y un _factor_ es que el último sólo puede tomar algunos valores permitidos (levels), con un orden interno predefinido (el cual ,por ejemplo, se respetará a la hora de realizar un gráfico)

Los valores de tipo factor permiten establecer un ordenamiento a los datos,  representado por una etiqueta.
...
unique(individual_t117$REGION)# Devuelve un listado de los diferentes valores que toma la columna en cuestión del dataframe
